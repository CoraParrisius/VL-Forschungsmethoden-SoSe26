...
<div id="aufgabe-container"></div>

<script>
  function loadAufgabe() {
    const now = new Date();

    const start = new Date("2026-04-23T14:55:00+02:00");
    const end   = new Date("2026-04-23T17:30:00+02:00");
    const msUntilStart = start - now;

    if (msUntilStart > 0) {
    setTimeout(() => location.reload(), msUntilStart);
    }

    let src;

    if (now >= start && now <= end) {
      src = "Integrierte Aufgaben/guetekriterium-wissenschaftlicher-erkenntnis-erkennen.html";
    } else {
      src = "Integrierte Aufgaben/guetekriterium-wissenschaftlicher-erkenntnis-erkennen.html";
    }

    document.getElementById("aufgabe-container").innerHTML = `
      <center>
        <iframe width="980" height="500" src="${src}" title="Aufgabe"></iframe>
      </center>
    `;
    
  }

  document.addEventListener("DOMContentLoaded", loadAufgabe);
</script>
...
::: {.column width='50%'}
2) Angenommen, die Forsch:innen sehen in den Daten, dass in diesen Klassen die Schüler:innen in einem Leistungstest tatsächlich besser abschneiden als in Klassen mit weniger formativem Feedback. Es kann dennoch nicht geschlussfolgert werden, dass das formative Assessment lernförderlicher ist. Warum?
:::
